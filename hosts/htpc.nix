{ config, lib, pkgs, ... }: {
  ...
  modules.multilogin = {
    enable = true;
    sessions = [
      "${pkgs.dbus}/bin/dbus-run-session XDG_SESSION_TYPE=wayland ${pkgs.gnome-session}/bin/gnome-session"
      ...
      "${pkgs.gamescope}/bin/gamescope --backend drm --immediate-flips --rt -- ${pkgs.pegasus-frontend}/bin/pegasus-fe"
